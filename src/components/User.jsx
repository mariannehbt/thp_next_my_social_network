...
const User = () => {
	...
	const userPostsList = posts.filter(elem => elem.user != null && elem.user.id === user.id).sort((a,b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime()).map((post, key) => (
		<div key={key}>
	...
	return (
		<div>
			...
			<button className='btn btn-outline-info' onClick={() => dispatch(API.fetchPosts())}>See posts</button>
			{userPostsList}
		</div>
	);
};
